...
confirm = ['multi', 'interphase', 'metaphase']
...
for tile in tiles:
    ...
    annotated_index = max(index for index, item in enumerate(identifier[-1]) if 'annotated' in item)
    identifier = identifier[:-1] + [identifier[-1][:annotated_index] + ['seg.npy']]    
...
#--------------------------construct images to show---------------------------#
            ...
            # make left_img 3 channels to match right_img
            left_img = np.dstack(3*[left_img])
