...
for i, tile in enumerate(tiles):
    ...
    bord = half_size+10
    filtered = (cv2.boxFilter((masks>0)*1.,-1,[350,350])>0)*1.
    filtered[:,:bord ] = 1
    filtered[:bord ,:] = 1
    filtered[:,-bord :] = 1
    filtered[-bord :,:] = 1
    while filtered.min()==0:
        # plt.cla()
        pp = np.unravel_index(np.argmin(filtered), filtered.shape)
        # original bounding box
        filtered[pp[0]-half_size:pp[0]+half_size,pp[1]-half_size:pp[1]+half_size] = 2
        X.append(raw[pp[0]-half_size:pp[0]+half_size,pp[1]-half_size:pp[1]+half_size])
        blanks += 1
        # plt.imshow(filtered, alpha=0.5)
        # plt.imshow((masks>0)*1.,alpha=0.5)
        # plt.pause(1)
        
    
#%% old method - find minimum distance
    # if num_masks == 0:
    #     blanks += 1
...
#%% show/save
y = np.array(['blank' for _ in X])

print(X.shape)
print(y.shape)

np.savez('/home/nel/NEL-LAB Dropbox/NEL/Datasets/smart_micro/datasets/blanks_fov200_0831.npz', X=X, y=y)
